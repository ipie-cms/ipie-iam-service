...
import in.gov.ipie.service.iam.event.UserVerifiedPayload;
import in.gov.ipie.service.iam.service.RoleService;

class KafkaUserVerifiedConsumerTest {

    private final RoleService roleService = mock(RoleService.class);
    private final ProcessedEventStore processedEventStore = mock(ProcessedEventStore.class);
    private final ObjectMapper objectMapper = new ObjectMapper();
    private final KafkaUserVerifiedConsumer consumer =
            new KafkaUserVerifiedConsumer(roleService, processedEventStore, objectMapper, "STAKEHOLDER");

    @Test
    void onEvent_assignsTheDefaultRole_forAMatchingEventType() {
        UUID userId = UUID.randomUUID();
        UUID keycloakUserId = UUID.randomUUID();
        EventEnvelope<UserVerifiedPayload> event = EventEnvelope.create(
                "USER_VERIFIED", 1, "ipie-user-service", null, null, new UserVerifiedPayload(userId, keycloakUserId, "a@b.com"));
