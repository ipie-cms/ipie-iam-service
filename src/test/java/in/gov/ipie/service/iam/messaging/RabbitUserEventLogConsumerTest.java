...
class RabbitUserEventLogConsumerTest {
    ...
    void onEvent_persistsAnAuditTrailRow_whenTheEventIsAnAuditEvent() {
        AuditEvent auditEvent = new AuditEvent(
                AuditEventType.BUSINESS, "ROLE_ASSIGNED", "USER", "user-1", null, "actor-1", "127.0.0.1",
                "ipie-iam-service", "onboarding", null, null, "corr-1", Instant.now());
        Map<String, Object> asMap = objectMapper.convertValue(auditEvent, Map.class);
        EventEnvelope<Map<String, Object>> event =
                EventEnvelope.create(OutboxAuditRecorder.AUDIT_EVENT_TYPE, 1, "ipie-iam-service", "corr-1", null, asMap);
        when(processedEventStore.isProcessed(event.eventId())).thenReturn(false);
