 ...
 * {@link UserCredentialJpaRepository} or {@link UserCredentialJpaEntity}.
 *
...
class UserCredentialRepositoryImpl implements UserCredentialRepository {
    ...
    public UserCredential save(UserCredential credential) {
        ...
        UserCredentialJpaEntity entity = jpaRepository.findById(credential.keycloakUserId())
                .map(existing -> {
                    existing.update(credential.passwordHash(), credential.algorithm(), credential.updatedAt());
                    return existing;
                })
                .orElseGet(() -> new UserCredentialJpaEntity(
    ...
    private static UserCredential toDomain(UserCredentialJpaEntity entity) {
