 ...
 * every audit row it produces, including in ipie-user-service (the originating action) - see
 * {@code PillarResolutionEventConsumer}'s own Javadoc for the cross-service case this
 * service's own rows participate in. Named {@code iam_audit_trail}, not {@code audit_trail} -
 * this service shares ipie-user-service's physical database (Database Mandatory Controls'
 * User/IAM shared-database exception), same table-prefixing convention as {@code
 * iam_idempotency_keys}/{@code iam_processed_events}/{@code iam_outbox_events}.
 */
@Entity
