...
class UserVerifiedConsumer {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(UserVerifiedConsumer.class);
    ...
    void onUserVerified(EventEnvelope<UserVerifiedPayload> event) {
        IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> {
            UserVerifiedPayload payload = event.data();
            LOG.info(
