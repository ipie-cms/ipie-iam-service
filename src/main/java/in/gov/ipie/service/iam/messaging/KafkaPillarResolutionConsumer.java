...
/**
 * Kafka counterpart to {@link PillarResolutionConsumer} - active only when this service
 ...
 * KafkaUserVerifiedConsumer}'s Javadoc for the same reasoning applied to a single-event
...
class KafkaPillarResolutionConsumer {
    ...
    void onEvent(EventEnvelope<?> event) {
        if (ACCOUNT_LINKED.equals(event.eventType())) {
            IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> handler.handleAccountLinked(
                    objectMapper.convertValue(event.data(), AccountLinkedPayload.class), event.correlationId()));
            ...
            IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> handler.handleAccountUnlinked(
                    objectMapper.convertValue(event.data(), AccountUnlinkedPayload.class), event.correlationId()));
        }
    }
}
