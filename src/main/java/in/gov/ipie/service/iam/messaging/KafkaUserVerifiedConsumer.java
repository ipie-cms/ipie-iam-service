...
/**
 * Kafka counterpart to {@link UserVerifiedConsumer} - auto-assigns the default role when
...
class KafkaUserVerifiedConsumer {

    private static final String EVENT_TYPE = "USER_VERIFIED";

    private static final Logger LOG = LoggerFactory.getLogger(KafkaUserVerifiedConsumer.class);
    ...
    void onEvent(EventEnvelope<?> event) {
        ...
        IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> {
            UserVerifiedPayload payload = objectMapper.convertValue(event.data(), UserVerifiedPayload.class);
            LOG.info(
