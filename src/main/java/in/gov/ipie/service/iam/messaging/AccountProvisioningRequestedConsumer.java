...
import in.gov.ipie.service.iam.event.AccountProvisioningRequestedPayload;
...
class AccountProvisioningRequestedConsumer {

    private static final Logger LOG = LoggerFactory.getLogger(AccountProvisioningRequestedConsumer.class);
    ...
    AccountProvisioningRequestedConsumer(
    ...
    @Transactional
    void onAccountProvisioningRequested(EventEnvelope<AccountProvisioningRequestedPayload> event) {
        IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> {
            AccountProvisioningRequestedPayload payload = event.data();
