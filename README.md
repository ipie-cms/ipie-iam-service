...
Owns authentication and authorisation: RBAC (roles, permissions, assignments), Keycloak account provisioning, and the stakeholder-resolution read path used on every stakeholder-SSO login.
...
- **root** — the Spring Boot service.
- **`keycloak-spi/`** — the custom Keycloak first-broker-login Authenticator. Not a service: no Spring Boot, no container. It deploys as a jar into Keycloak's providers directory.

They live together because they implement one feature — stakeholder SSO federation — across the `POST /internal/stakeholder-links/resolve` contract, HMAC-signed by the SPI and verified by this service's `HmacSignatureVerificationFilter`. That contract changes on both sides at once, and the SPI carries a cross-check test asserting its hand-written HMAC stays byte-identical to `common-security`'s. Separate repositories would mean two coordinated releases for one change.
